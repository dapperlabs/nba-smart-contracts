...
pub contract TopShot: TopShotPublic {
    ...
    pub resource Moment {
        ...
        init(newID: Int, moldID: Int, quality: Int, place: Int, reference: &TopShotPublic) {
            pre {
                ...
                //place > reference.molds[moldID]?.qualityCounts[quality]:  "cannot mint more than the amount in quality!"
    ...
    pub resource MomentMinter {
        ...
        pub let topShotRef: &TopShot
        pub let pubTopShotRef: &TopShotPublic

        init(ref: &TopShot, pubref: &TopShotPublic) {
            self.momentID = 1
            self.topShotRef = ref
            self.pubTopShotRef = pubref
        }
        ...
        pub fun mintMoment(moldID: Int, quality: Int, recipient: &MomentReceiver) {
            pre {
                ...
                self.topShotRef.getNumMomentsLeftInQuality(id: moldID, quality: quality) <= 0: "All the moments of this quality have been minted!"
            }

            // update the number left in the quality that are allowed to be minted
            self.topShotRef.molds[moldID]?.updateNumLeft(quality: quality)

            // gets this moment's place in the moments for this quality
            let placeInQuality = self.topShotRef.getNumMintedInQuality(id: moldID, quality: quality)
    ...
    init() {
        ...
        // need to be able to create a reference to self in the init function
        let oldMinter <- self.account.storage[MomentMinter] <- create MomentMinter(ref: &self, pubref: &self as TopShotPublic)
        destroy oldMinter

        self.account.storage[&MomentMinter] = &self.account.storage[MomentMinter] as MomentMinter
    }

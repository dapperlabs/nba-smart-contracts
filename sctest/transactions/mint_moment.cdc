...
transaction {
    ...
    prepare(acct: Account) {
        ...
        let moment1 <- acct.storage[TopShot.MomentMinter]?.mintMoment(moldID: 0, quality: 1) ?? panic("No minter!")
        let moment2 <- acct.storage[TopShot.MomentMinter]?.mintMoment(moldID: 1, quality: 2) ?? panic("No minter!")
